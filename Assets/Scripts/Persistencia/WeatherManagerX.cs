...
public class WeatherManager : MonoBehaviour
{
    ...
    public DailyWeather GenerateWeather()
    {
        ...
        // Añadir más eventos “malos/buenos” según el éxito del jugador
    ...
    private void PlayWeatherSFX(DailyWeather w)
    {
        ...
        switch (w.type)
        {
            ...
            case WeatherType.Lluvia:
                ...
                break;
            case WeatherType.Granizo:
                // Si tenéis clip específico de granizo, cambiad a PlayTormenta() o similar
    ...
    // ======== GETTERS/SETTERS para guardado/carga ========

    public DailyWeather GetCurrentWeather() => currentWeather;

    public List<DailyWeather> GetForecastList()
    {
        var list = new List<DailyWeather>();
        for (int i = 0; i < forecast.Length; i++) list.Add(forecast[i]);
        return list;
    }

    public void SetCurrentWeather(DailyWeather w)
    {
        currentWeather = w;
        OnCurrentWeatherChanged?.Invoke(currentWeather);
        PlayWeatherSFX(currentWeather);
    ...
    public void SetForecast(List<DailyWeather> list)
    {
        for (int i = 0; i < forecast.Length && i < list.Count; i++)
            forecast[i] = list[i];
